...
lm2.1 = lm(price ~  landValue+livingArea+bathrooms+ waterfront +centralAir, data=saratoga_train)
...
The best model is found at k = 19, with an RMSE of just above 56,000. This also outperforms our best linear model. Therefore we believe that K-nearest-neighbor regression should be used to predict taxable market values.
...
We don't think this dataset is appropriate for building a predictive model of defaults. Instead, we would recommend the bank use a dataset consisting of a random sample of loans in the bank's overall portfolio.
...
set.seed(1)
hlasso <- cv.gamlr(x, y, nfold=10, family="binomial")
...
# RMSE table
RMSE3 = tibble("model" = c("lm1 (baseline 1)", "lm2", "lm3 (baseline 2)", "lm4", "lm5", "lm_lasso"), "RMSE" = c(rmse(lm1, Hdev_test), rmse(lm2, Hdev_test), rmse(lm3, Hdev_test), rmse(lm4, Hdev_test), rmse(lm5, Hdev_test), rmse(lm_lasso, Hdev_test)))
RMSE3.1 = kable(RMSE3, title = "RMSE of each model, first pass")
RMSE3.1
...
RMSE4 = tibble("model" = c("lm4", "lm5", "lm_lasso"), "RMSE" = colMeans(rmse_sim))
RMSE4.1 = kable(RMSE4, title = "Average RMSE of three best models through 10 train/test splits")
RMSE4.1
```
