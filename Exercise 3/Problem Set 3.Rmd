...
```{r 3.2}
# fit a single tree
buildings.tree = rpart(Revenue ~ .,
                        data=buildings_train, control = rpart.control(cp = 0.00001))
modelr::rmse(buildings.tree, buildings_test)


# ten times iterations for finding the average RMSE
...
Our measure for comparing the results are their RMSEs. Based on the results of the simulations, the random forest model included all variable has the lowest out of the sample RMSE. The other random forest has the second-best result, which shows proves the better performance of the random forest comparing the other decision trees models. So, to predict the revenue per square foot per calendar year, we will apply the comprehensive random forest model. 
