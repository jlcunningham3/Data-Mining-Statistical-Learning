...
greenbuildings <-greenbuildings%>%
  mutate( Revenue = leasing_rate * Rent, 
          green_certification = ifelse(green_rating, "Pass", "Fail"),
...
ggplot(greenbuildings) + 
  geom_boxplot(aes(x = green_certification, y =Revenue))+ 
...
lm4=lm(Revenue ~ .,data= buildings_train)

set.seed(3)
# fit a single tree
buildings.tree0 = rpart(Revenue ~ .,
                        data=buildings_train, control = rpart.control(cp = 0.00001))
modelr::rmse(buildings.tree0, buildings_test)

set.seed(3)
# fit Random Forest
buildings.forest0 = randomForest(Revenue ~ ., data=buildings_train,  na.action = na.exclude)

buildings.forest1 = randomForest(Revenue ~ City_Market_Rent + Electricity_Costs + size + stories + age + green_rating, data=buildings_train, importance = TRUE)

buildings.forest2 = randomForest(Revenue ~ City_Market_Rent + age + renovated + amenities + class_a + class_b + LEED + Energystar + total_dd_07, data=buildings_train, importance = TRUE)

# out-of-sample fit
RMSE4 = tibble("model" = c("lm (baseline)", "buildings.tree0", "buildings.forest0", "buildings.forest1", "buildings.forest2"), "RMSE" = c(rmse(lm4, buildings_test), rmse(buildings.tree0, buildings_test), rmse(buildings.forest0, buildings_test), rmse(buildings.forest1, buildings_test), rmse(buildings.forest2, buildings_test)))
RMSE4.1 = kable(RMSE4, caption = "RMSE of each model, first pass")
RMSE4.1
...
# on the outcome partialPlot(buildings.forest, buildings_test, 'City_Market_Rent', las=1)
partialPlot(buildings.forest1, buildings_test, 'City_Market_Rent', las=1)
partialPlot(buildings.forest1, buildings_test, 'green_rating', las=1)

p1 = pdp::partial(buildings.forest1, pred.var = 'City_Market_Rent', n.trees=350)
ggplot(p1) + geom_point(mapping=aes(x= City_Market_Rent, y=yhat))

p2 = pdp::partial(buildings.forest1, pred.var = 'green_rating', n.trees=350)
ggplot(p2) + geom_point(mapping=aes(x=green_rating, y=yhat))
```
...
boost_buildings = gbm(Revenue ~ City_Market_Rent + age+ renovated + amenities+class_a+class_b + green_rating + total_dd_07, data = buildings_train, distribution='gaussian', interaction.depth=4, n.trees=500, shrinkage=.05)

#  error curve -- 
gbm.perf(boost_buildings)

yhat_test_gbm = predict(boost_buildings, buildings_test, n.trees=100)

# RMSE
rmse(boost_buildings, buildings_test)

plot(boost_buildings, 'City_Market_Rent')
plot(boost_buildings, 'green_rating')
```
